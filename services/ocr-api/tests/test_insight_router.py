...
def test_sub_threshold_candidates_are_dropped():
    # Floor is 0.3; a 0.2 signal must not dispatch its agent.
    cands = [
...
def test_severity_floor_constant_matches_detector_floor():
    # Detectors clamp to >= 0.3; the router floor must stay aligned.
    assert DEFAULT_SEVERITY_FLOOR == 0.3


def test_engagement_weight_can_drop_dismissed_type():
    # Phase 4 hook: weight 0.5 on a barely-above-floor streak drops it.
    cands = [_c("streak", "streak:3", severity=0.35)]
