...
Full v1 product spec (still mostly accurate for the core capture flow, but the app has since grown well past its scope — see `docs/system/decisions.md` for what changed): `docs/archive/superpowers/specs/2026-05-11-receipt-drop-design.md`.
...
- **OCR**: self-hosted `services/ocr-api` — Python/FastAPI + Tesseract (`pytesseract`). Not on-device, not PaddleOCR — both were tried and replaced (`docs/system/decisions.md`).
...
- **Maps**: `google_maps_flutter` (real Google Maps SDK). Briefly on `flutter_map` + free CARTO Voyager tiles due to a GCP billing blocker, reverted once billing was enabled — see `docs/system/decisions.md` ("Back to `google_maps_flutter`", 2026-07-09). Custom pins (spend places, friends) are Flutter widgets overlaid on the map, not native `Marker`s — see `docs/system/decisions.md`'s 2026-07-13 entry for the viewport-query/clustering design built on top of that.
- **`Impact Drops/`**: a separate, frozen React/TanStack Start prototype (Lovable-generated) used only as a one-time design/behavior reference for the gamification features. **Not built, imported, or deployed with the product.** See `docs/system/architecture.md`.
...
Full diagram, data-flow trace, and design patterns: **`docs/system/architecture.md`**.

## Which files to read for which task

| Task | Read first |
|---|---|
| Understand the whole system | This file → `docs/system/architecture.md` → `.claude/context.md` |
| Change the receipt capture/OCR/parse pipeline | `docs/system/architecture.md` (data flow section) → `memory/feature_graph.md` (Receipt capture feature) → the actual files under `lib/features/share/` and `lib/domain/logic/` |
| Change the DB schema | `docs/database/schema.md` (cumulative schema + RLS) → the latest 2-3 files in `supabase/migrations/` for the current pattern to follow |
| Add/change an API endpoint or Edge Function | `docs/api/overview.md` |
| Understand why something is built the way it is | `docs/system/decisions.md` |
| Understand enrich-transaction flow | `docs/api/enrich-transaction.md` |
| OCR preprocessing improvements (planned) | `docs/plans/2026-07-21-ocr-preprocessing-pipeline-upgrade.md` |
...
| Understand a past technical pivot (OCR engine, maps SDK, leaderboard split) | `memory/experiments.md` and `docs/system/decisions.md` |
| Anything involving `Impact Drops/` | `docs/system/architecture.md` "mental model" section first — it's a reference, not live code |
...
docs/
  ...
  api/
    overview.md               Edge Functions + both Python services
    enrich-transaction.md     enrichment Edge Function walkthrough
  database/
    schema.md                 Postgres schema, RLS, storage, RPCs, Drift
  plans/
    *.md                      implementation plans (in-flight work)
  design/
    design_handoff_receipt_flows/  UI handoff mocks
  archive/
    superpowers/              original v1 spec + plan (historical)
.claude/
  context.md                  current in-flight state, incomplete features
...
Legacy top-level paths (`docs/architecture.md`, `docs/api.md`, etc.) redirect to the nested locations above.

Keep this system current: when you make an architecturally significant change (new table, new service, a reversed decision), update the relevant `docs/` file and add an entry to `docs/system/decisions.md` or `memory/experiments.md` rather than letting this fall out of sync with the code.
