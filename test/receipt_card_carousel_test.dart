...
void main() {
  testWidgets('shows an empty state when there are no receipts',
      (tester) async {
    await tester.pumpWidget(_harness(const ReceiptCardCarousel(transactions: [])));
    await tester.pump();
  ...
  testWidgets(
      'a single receipt renders the newest badge and hides the dots',
      (tester) async {
    final tx = _tx(id: 'a', occurredAt: DateTime(2026, 7, 10), merchant: 'Solo Cafe');
  ...
  testWidgets('multiple receipts show one dot per receipt and only one card',
      (tester) async {
    final txs = [
      _tx(id: 'a', occurredAt: DateTime(2026, 7, 10, 12), merchant: 'Newest Spot'),
      _tx(id: 'b', occurredAt: DateTime(2026, 7, 10, 11), merchant: 'Older Spot'),
      _tx(id: 'c', occurredAt: DateTime(2026, 7, 10, 10), merchant: 'Oldest Spot'),
    ];
