...
- Input: `{ transaction_id }`. Fetches the row (`raw_ocr_text`/`ocr_header_text`, `share_location_lat/lng`, `place_status`), then:
  ...
  - **LLM receipt understanding** (`supabase/functions/_shared/receipt_understanding.ts`): calls `services/ocr-api`'s `POST /understand` (shared-secret auth, same `OCR_SERVICE_URL`/`OCR_SERVICE_SECRET` as `ocr-proxy`) with the row's OCR text (`raw_ocr_text`, falling back to `ocr_header_text`). That endpoint is the one that actually talks to the LLM gateway — see `docs/decisions.md` for why the LLM call itself lives in `services/ocr-api` rather than this edge function. The response is a structured `{merchant_name, merchant_search_queries, address_text, location_clues, vendor_category, google_place_types, confidence}`, re-validated here via `parseReceiptUnderstanding()` as a second line of defense. **No fallback** — this is a testing-phase design decision: any failure (timeout, non-2xx, unparseable JSON) writes `pipeline_status='failed_enrichment'` and `llm_understanding={_error, _raw, _meta}`, and the request stops there; it does **not** fall back to the old heuristic merchant-candidate matching. On success the structured output is persisted to `llm_understanding` (plus `_meta`: latency/prompt_source) and `merchant_normalized` is set from `merchant_name` before any Places call, so it's inspectable even if a later step fails.
...
Used only by `enrich-transaction`. `callReceiptUnderstanding(cfg, ocrText, fetchFn?)` posts `{ocr_text}` to `${OCR_SERVICE_URL}/understand` with `X-OCR-Secret` (the actual LLM call happens server-side in `services/ocr-api`, not here) and returns either `{ok:true, understanding, raw, latencyMs}` or `{ok:false, error, raw, latencyMs}` — no retry on failure. `parseReceiptUnderstanding(raw)` re-validates whatever ocr-api returns (defense-in-depth against a version-skew/malformed proxy response, since ocr-api already validates before responding): strips `<think>` blocks/markdown fences, coerces every field, drops off-vocabulary categories/place-types, clamps confidences, dedupes/caps search queries at 3; returns `null` only when nothing actionable survives. `buildLlmTextQueries()` and `resolveIncludedTypes()` turn the validated understanding into Places `textQuery`/`includedTypes` inputs; `adjustScoreForTypeMatch()` penalizes type-mismatched candidates post-scoring. These stay in Deno (not moved to Python) since they're tightly coupled to the Places API call shape built in this same edge function. Unit-tested (no network) in `receipt_understanding.test.ts`; the LLM-calling/prompt logic itself is tested separately in `services/ocr-api/tests/test_receipt_understanding.py`.
...
| POST | `/understand` | `X-OCR-Secret` header (same shared secret) | Body = `{ocr_text: str}`. LLM receipt-understanding step — see below. Returns the structured understanding JSON, or `400 empty_ocr_text` / `500 server_misconfigured` (`VLLM_BASE_URL`/`VLLM_MODEL_NAME` unset) / `504 llm_timeout` / `502 llm_http_*`\|`llm_invalid_response_json`\|`llm_unparseable_content`. |
...
**`/understand` (`app/receipt_understanding.py`)**: takes raw OCR text (from `enrich-transaction`, reading the already-synced `transactions.raw_ocr_text`/`ocr_header_text` — not a live OCR call), builds a fixed system prompt (Malaysian-receipt-specific: corrects OCR spelling, strips legal suffixes for search queries, infers category from line items, excludes phone/receipt/tax numbers), and calls a self-hosted OpenAI-compatible gateway (`VLLM_BASE_URL`/`VLLM_MODEL_NAME` required, `VLLM_API_KEY`/`VLLM_REASONING_EFFORT` optional — same vars the root `.env`'s `scripts/inspect_llm_endpoint.py` probe uses) via a shared `httpx.AsyncClient` (created once at app startup via FastAPI `lifespan`, not per-request). `response_format: json_object` is sent as a hint with one retry without it on HTTP 400 (some LiteLLM/vLLM backends reject the param); `parse_receipt_understanding()` is the actual validation authority — strips `<think>` blocks/markdown fences, coerces every field, drops off-vocabulary `vendor_category`/`google_place_types`, clamps confidences, caps search queries at 3. **No fallback**: any failure (timeout, non-2xx, unparseable content) raises `ReceiptUnderstandingError`, mapped to an HTTP error by `receipt_understanding_error_handler` — never a weaker heuristic result. See `docs/decisions.md` for why this call lives here instead of directly in `enrich-transaction`.
...
Consumers: `lib/features/share/ocr_api_client.dart` (Flutter direct-call, dev only), `ocr-proxy` edge function (production OCR path), `bin/process_receipts.dart` (batch CLI, calls `/ocr` directly, never through the proxy), `enrich-transaction` edge function (calls `/understand` directly — no proxy, it's already server-side).

Tests: `cd services/ocr-api && pytest` — `tests/test_receipt_understanding.py` covers `/understand` and its parsing/LLM-call logic (`httpx.MockTransport`, no real network/gateway).
...
| LLM gateway (self-hosted, OpenAI-compatible) | `services/ocr-api` (`POST /understand`, called by `enrich-transaction`) | Structures raw OCR text into merchant name/queries/address/category before Places matching. No fallback — a failed call fails the enrichment. |
