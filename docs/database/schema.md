...
| `place_geom` | `geometry(Point, 4326)`, generated (stored) | added `20260713000000` — originally `ST_MakePoint(coalesce(place_lng, share_location_lng), coalesce(place_lat, share_location_lat))`; redefined `20260728100000` to drop the `share_location` fallback (`case when place_lat is not null and place_lng is not null then ST_MakePoint(place_lng, place_lat) else null end`) — map pins use resolved place_lat/lng only, share_location is enrichment-fallback-only and no longer displayed on the spend map. Null when place_lat/place_lng aren't both set. Requires the `postgis` extension (enabled in the same migration). Bounding-box viewport queries only (`&&`/`ST_MakeEnvelope`), not radius/distance search — geometry (not geography) was chosen for that reason |
...
- `bill_splits`: `id`, `transaction_id` FK → `transactions` (cascade, **unique** — one split per receipt), `owner_id` FK → `profiles` (cascade), `mode text` (`equal|by_item`), `total_myr` snapshotted at creation and rewritten when the payer edits the receipt amount / line items (`20260813140000`), `created_at`. RLS: select/insert/delete owner-only; **update** owner-only for amount rewrite (composition who/mode stays insert-or-delete; no edit-who UI).
...
| `get_map_transactions_in_bounds(min_lat, min_lng, max_lat, max_lng, start_at, end_at, category)` | `security definer` | Caller's own geolocated transactions inside a lat/lng bounding box + time/category filter, via the `place_geom` GiST index; explicit `where user_id = auth.uid()` since security definer bypasses RLS; `limit 2000`. Returns per-row `line_items` (jsonb of `{id, name, price_myr, quantity}`) + `receipt_storage_path` (latest `receipt_artifacts.storage_path`) so the Receipt Map Sheet can render items/photo and join by-item split assignees without a second round-trip — detail columns in `20260811180000` (DROP + recreate); `id` added inside jsonb in `20260812140000`; `20260814000000` dropped a dead `coalesce(..., share_location_lat/lng)` fallback in the returned `lat`/`lng` (unreachable since `place_geom`'s `20260728100000` redefinition already requires non-null `place_lat`/`place_lng`) | `20260713000000`, detail columns in `20260811180000`, line-item `id` in `20260812140000`, dead-fallback cleanup in `20260814000000` | `map_transactions_repository.dart` (spend map own-place pins, fetched on `onCameraIdle` only) |
