...
- `get_my_split_requests()` — `security definer`, joins `bill_split_participants` (`where friend_user_id = auth.uid()`) → `bill_splits` → `transactions` (for `merchant_raw`) → `profiles` (payer's name/avatar). The only cross-user read in this feature — `transactions`/`profiles` are otherwise owner-only-readable, same reasoning as `get_friend_feed()`.
