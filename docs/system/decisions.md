...
**Reason**: no bill-splitting feature existed in any form before this. Friend groups needed to be a real table (not an ad-hoc per-split selection) because the source design's quick-select group chips are meant to be reusable across future splits, not recreated each time. Friend-side visibility was added because this app has no push-notification system — without an in-app surface, a friend would have no way to ever learn a payer had split a bill with them; the Home banner (mirroring the existing pending-imports/needs-review banner pattern) plus `SplitRequestsScreen` is the only discovery path. `receipt_line_items.id` had to be surfaced through `ReceiptLineItem`/`TransactionView` (previously dropped in `transaction_repository_native.dart`'s `_lineItemsFor()`) so `bill_split_item_assignments.line_item_id` could FK to a real, stable id instead of a positional index.
